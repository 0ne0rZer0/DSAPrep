...
int main()
{
    ...
    for(j=1; j<=t; j++)
    {
        ...
        vector<vector<pair<ll, ll>>> in(d+2);
        vector<vector<pair<ll, ll>>> out(d+2);
        ...
        for(i=1; i<=d; i++)
        {
            ...
            for(i1=0; i1<l; i1++)
            {
                available.insert(make_pair(in[i][i1].first, in[i][i1].second));
            }
            ...
            for(i1=0; i1<l; i1++)
            {
                available.erase(make_pair(out[i][i1].first, out[i][i1].second));
                
                p=top_k.erase(make_pair(out[i][i1].first, out[i][i1].second));
